...
namespace UE4
{
	struct GWorld
	{
		...
		uintptr_t ULevel(uintptr_t World) {
			return driver.readv<uintptr_t>(World + offsets::Ulevel);
		};
	};
	...
	struct GPrivatePawn {
		uintptr_t USkeletalMeshComponent(uintptr_t Pawn) {
			return driver.readv<uintptr_t>(Pawn + offsets::MeshComponent);
		};
	};
	...
	struct GLocalPlayer {
		uintptr_t APlayerController(uintptr_t ULocalPlayer) {
			return driver.readv<uintptr_t>(ULocalPlayer + offsets::PlayerController);
		};
	};

	struct GPlayerController {
		uintptr_t APlayerCameraManager(uintptr_t APlayerController) {
			return driver.readv<uintptr_t>(APlayerController + offsets::PlayerCameraManager);
		};
		uintptr_t AHUD(uintptr_t APlayerController) {
			return driver.readv<uintptr_t>(APlayerController + offsets::MyHUD);
		};
		uintptr_t APawn(uintptr_t APlayerController) {
			return driver.readv<uintptr_t>(APlayerController + offsets::AcknowledgedPawn);
		};
	};
	...
	namespace SDK
	{
		...
		auto ProjectWorldToScreen(FVector WorldLocation) -> FVector
		{
			...
			auto ViewInfo = driver.readv<FMinimalViewInfo>(PlayerCameraManager +0x1F80 + 0x10);
